...
# TurtleBot 网络 (输出 linear_vel, angular_vel)
from turtlebot_net_ori import TurtleBotNet, FinalNet
# 支持两种数据加载器：固定帧数 vs 动态帧数
# 使用 --dynamic-loader 参数切换
from turtlebot_loader_ddp import CarlaH5DataDDP
from turtlebot_loader_dynamic import CarlaH5DataDDP as CarlaH5DataDDPDynamic
...
# TurtleBot 特定参数
parser.add_argument('--max-linear-vel', default=0.7, type=float,
                    help='max linear velocity for normalization (m/s)')
parser.add_argument('--max-angular-vel', default=1.0, type=float,
                    help='max angular velocity for normalization (rad/s)')
