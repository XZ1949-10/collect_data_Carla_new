...
# coding=utf-8
'''
TurtleBot 推理主模块 (直接预测 linear_vel 和 angular_vel)

与原版本的区别:
- 使用 TurtleBotPredictor 直接预测速度
- 不需要 CARLA 格式转换
- 输出更简洁
'''
...
from .config.inference_config import (
    CONTROL_RATE_HZ, DEFAULT_COMMAND, COMMAND_NAMES, 
    SPEED_NORMALIZATION_KMH, TURTLEBOT_PARAMS
)
from .model.model_loader import ModelLoader
from .model.turtlebot_predictor import TurtleBotPredictor
...
class TurtleBotDirectInference:
    """
    TurtleBot 直接推理类
    
    直接预测 linear_vel 和 angular_vel，无需 CARLA 格式转换
    """
    ...
    def load_model(self, network_module=None):
        ...
        参数:
            network_module: 网络模块类，如果为 None 则尝试从默认路径导入
        """
        self.model = self.model_loader.load(network_module)
        self.predictor = TurtleBotPredictor(
            self.model, self.device, self.turtlebot_model
        )
        print("✅ 模型加载完成")
    ...
    def run(self, duration=None, print_interval=10):
        ...
        print(f"\n{'='*60}")
        print("开始推理 (直接预测 linear_vel, angular_vel)")
        print(f"控制频率: {self.control_rate} Hz")
        print(f"TurtleBot 型号: {self.turtlebot_model}")
        print(f"max_linear: {self.max_linear} m/s")
        print(f"max_angular: {self.max_angular} rad/s")
        print(f"{'='*60}\n")
        ...
        try:
            while not rospy.is_shutdown() and self.command_controller.is_running():
                ...
                # 模型预测 (直接得到 linear_vel 和 angular_vel)
                result = self.predictor.predict(
                    img_tensor, speed_normalized, current_command
                )
    ...
    def _print_status(self, start_time, speed_data, result, current_command):
        ...
        print(f"[{elapsed:.1f}s] "
              ...
              f"Lin: {result['linear_vel']:+.3f} m/s | "
              f"Ang: {result['angular_vel']:+.3f} rad/s | "
              f"FPS: {fps:.1f}")
