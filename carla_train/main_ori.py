...
parser.add_argument('--branch-weight', default=1, type=float,
                    help='branch weight')
parser.add_argument('--id', default="ziji_demo", type=str)
parser.add_argument('--train-dir',
                    default=r"./carla_data/AgentHuman/SeqTrain",
                    type=str, metavar='PATH',
                    help='training dataset')
parser.add_argument('--eval-dir',
                    default=r"./carla_data/AgentHuman/SeqVal",
                    type=str, metavar='PATH',
...
def main():
    ...
    pretrained_path = "./save_models/test_demo_best.pth"
    if os.path.exists(pretrained_path):
	    ...
	    try:
	        ...
	        # 移除前缀
	        new_state_dict = {k.replace('module.carla_net.', ''): v 
	                         for k, v in state_dict.items()}
	        
	        # 加载参数
	        model.carla_net.load_state_dict(new_state_dict)
	        ...
	        # 可选：重新初始化模型参数
	        # model.apply(weights_init)  # 如果你需要
    else:
    ...
    # Create optimizer before wrapping with DataParallel
        ...
        for i, (img, speed, target, mask) in enumerate(loader):
            ...
            if args.net_structure != 1:
                ...
                uncertain_loss = args.branch_weight * branch_loss \
                    + args.speed_weight * speed_loss
                uncertain_losses.update(uncertain_loss.item(), args.batch_size)
                ori_losses.update(uncertain_loss.item(), args.batch_size)
